...
contract SIGN is  Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using ECDSA for bytes32;

    mapping(address => uint256) public nonceUser;

    address public SIGNER = address(0x1);//签名地址
    ...
    function claim(uint256 tokenAmount,uint256 nonce,uint256 timestamp, bytes memory sig) public nonReentrant() {

        require(nonceUser[msg.sender] == nonce, "nonce error");
        require(block.timestamp <= timestamp + timeout,"sig timeout");

        bytes32 hash = keccak256(abi.encodePacked(tokenAmount, nonce, timestamp));
        require(
